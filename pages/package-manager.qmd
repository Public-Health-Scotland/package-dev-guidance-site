...
PHS have an instance of [Posit Package Manager](https://posit.co/products/enterprise/package-manager/){target="_blank"}, - [https://ppm.publichealthscotland.org/client/](){target="_blank"} this provides a seamless interface for users when installing packages either from CRAN, or packages hosted elsewhere - this is how the PHS packages can be installed easily even though they are only hosted on GitHub. 
...
Commit mode is simpler as it requires less work from package maintainers but is not recommended. Tag mode is more suitable as it will only expose releases of packages to users when the maintainer decides.
